...
test('Search, filter, add product to cart, then remove from cart', async ({ page }) => {
  ...
  const BRAND = 'Gadpiparty';
  const QTY = 2;

  // Navigate to main page
  const main_page = new MainPage(page);
  await main_page.goToUrl();
  await expect(page).toHaveURL(main_page.url);
  ...
  const filtered_products_counts = await search_results_page.filteredProductsLinks.count();
  const assertions: Promise<void>[] = [];
  for (let i = 0; i < filtered_products_counts; ++i) {
    assertions.push(
      expect(search_results_page.filteredProductsLinks.nth(i)).toHaveText(BRAND)
    );
  }
  ...
  await expect(product_page.productTitle).toContainText(BRAND);

  // Add a number of the product to cart
  await product_page.addToCart(QTY);
  const post_add_cart_page = new postAddCartPage(page);
  await Promise.all([
    expect(post_add_cart_page.addedToCartHeading).toBeVisible(),
    expect(post_add_cart_page.header.cartCount).toHaveText(`${QTY}`)
  ]);

  // Remove from cart
  await post_add_cart_page.removeProduct();
  await expect(post_add_cart_page.header.cartCount).toHaveText('0');
});
